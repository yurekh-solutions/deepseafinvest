...
import { ArrowRight, Play, TrendingUp, Shield, PiggyBank, BarChart3, Users, Building2 } from 'lucide-react';
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <div className={`space-y-8 ${mounted ? 'animate-slide-up' : 'opacity-0'}`}>
            ...
            <div className="inline-flex items-center gap-2 px-4 py-2 glass-card rounded-full">
              ...
              <span className="text-white/80 text-sm font-medium">Trusted Wealth Partner Since 2011</span>
            </div>
            ...
            <p className="text-lg sm:text-xl text-white/70 max-w-xl leading-relaxed">
              Expert financial guidance across Insurance, Mutual Funds, Fixed Income & Equity. 
              Mumbai-based with global reach in Singapore.
            </p>

            {/* CTA Buttons */}
            <div className="flex flex-col sm:flex-row gap-4">
              <Link href="/contact" className="glass-button px-8 py-4 rounded-xl text-white font-semibold flex items-center justify-center gap-2 group">
                Start Your Journey
                <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
              </Link>
              <Link href="#services" className="glass-button-secondary px-8 py-4 rounded-xl text-white font-semibold flex items-center justify-center gap-2 group">
          ...
          <div className={`relative ${mounted ? 'animate-slide-right' : 'opacity-0'}`} style={{ animationDelay: '0.3s' }}>
            {/* Background Glow */}
            ...
            <div className="relative glass-card-elevated rounded-3xl overflow-hidden card-hover-lift">
              ...
              <div className="p-6 space-y-4">
                <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${services[activeService].color} flex items-center justify-center`}>
                  {(() => {
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              {services.filter((_, i) => i !== activeService).slice(0, 2).map((service, index) => (
                <Link
                  key={service.title}
                  href={service.href}
