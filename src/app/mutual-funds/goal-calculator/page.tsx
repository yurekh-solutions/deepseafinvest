...
export default function GoalCalculatorPage() {
  ...
  const futureValueOfSavings = existingSavings * Math.pow(1 + expectedReturn / 100, years);
  
  // Remaining amount needed
  const remainingAmount = Math.max(0, goalAmount - futureValueOfSavings);
  
  // Required monthly SIP
  const requiredSIP = remainingAmount > 0 
  ...
  return (
    ...
    >
      ...
      <div className="max-w-3xl mx-auto text-center mb-12">
        <h2 className="text-2xl md:text-3xl font-bold text-white mb-4">Financial Goal Planning Calculator</h2>
        <p className="text-white/70 leading-relaxed mb-4">
          Whether your goal is buying a home, funding higher education, or building an emergency corpus — a goal-based investment approach ensures you stay on track. By working backwards from your target amount, you can calculate exactly how much to invest each month to reach your goal on time.
        </p>
        <p className="text-white/60 leading-relaxed mb-6">
          Our Goal Calculator factors in inflation, your investment timeline, and expected returns to give you a precise monthly SIP figure. Set clear financial milestones and let systematic investing do the rest.
        </p>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-left">
          {[{label:'Accounts for', value:'Inflation impact'},{label:'Returns assumption', value:'8–15% p.a.'},{label:'Goals covered', value:'Home, Education, Travel'}].map(item => (
      ...
      <div className="grid lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
        ...
        <GlassCard className="p-8" elevated>
          ...
          <div className="space-y-6">
            {/* Goal Amount */}
            <div>
              <label className="block text-white/70 mb-2">Target Amount: {formatCurrency(goalAmount)}</label>
              <input
                type="range"
                min="100000"
                max="10000000"
            ...
            {/* Years */}
            <div>
              <label className="block text-white/70 mb-2">Time to Goal: {years} years</label>
              <input
                type="range"
                min="1"
                max="30"
            ...
            {/* Existing Savings */}
            <div>
              <label className="block text-white/70 mb-2">Existing Savings: {formatCurrency(existingSavings)}</label>
              <input
                type="range"
                min="0"
                max="5000000"
        ...
        <div className="space-y-6">
          <GlassCard className="p-8" elevated>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center py-3 border-b border-glass-border">
                ...
                <span className="text-white font-semibold">{formatCurrency(goalAmount)}</span>
              </div>
              ...
              <div className="flex justify-between items-center py-3 border-b border-glass-border">
                ...
                <span className="text-white font-semibold">{formatCurrency(futureValueOfSavings)}</span>
              </div>
              ...
              <div className="flex justify-between items-center py-3 border-b border-glass-border">
                ...
                <span className="text-accent-red font-semibold">{formatCurrency(remainingAmount)}</span>
              </div>
          ...
          <GlassCard className="p-6">
            <h3 className="text-lg font-semibold text-white mb-4">Common Goals</h3>
            <div className="space-y-3">
              <button 
                onClick={() => { setGoalAmount(2000000); setYears(5); }}
              ...
              <button 
                onClick={() => { setGoalAmount(5000000); setYears(10); }}
                className="w-full text-left p-3 bg-white/5 rounded-lg hover:bg-white/10 transition-colors"
              >
              ...
              <button 
                onClick={() => { setGoalAmount(10000000); setYears(15); }}
                className="w-full text-left p-3 bg-white/5 rounded-lg hover:bg-white/10 transition-colors"
              >
                <p className="text-white font-medium">Dream Home Down Payment</p>
